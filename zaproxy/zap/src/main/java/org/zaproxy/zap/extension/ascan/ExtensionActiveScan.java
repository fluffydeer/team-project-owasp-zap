...
public class ExtensionActiveScan extends ExtensionAdaptor
    ...
    public void postInit() {
        ...
        if (Control.getSingleton().getMode().equals(Mode.attack)) {
            if (hasView() && !this.getScannerParam().isAllowAttackOnStart()) {
                // Disable attack mode for safeties sake (when running with the org.zaproxy.zap.extension.policyRuleVerifier.UI)
    ...
    public int startScan(String name, Target target, User user, Object[] contextSpecificObjects) {
        ...
        if (hasView()) {
            ...
            scanner.addScannerListener(getActiveScanPanel()); // So the org.zaproxy.zap.extension.policyRuleVerifier.UI get updated
    ...
    public void stopScan(int id) {
        ...
        // Dont need to update the org.zaproxy.zap.extension.policyRuleVerifier.UI - this will happen automatically via the events
    }
    ...
    public void pauseScan(int id) {
        ...
        if (hasView()) {
            // Update the org.zaproxy.zap.extension.policyRuleVerifier.UI in case this was initiated from the API
